...
Ce projet a pour objectif de concevoir une chaîne complète de traitement de données, de la collecte initiale jusqu'à la visualisation dans un dashboard fonctionnel. Le sujet choisi porte sur l'analyse de la qualité nutritionnelle des produits alimentaires en s'appuyant sur les données d'OpenFoodFacts. [Consulter l'énoncé du TP (PDF)](TP_DataS1.pdf)
...
* **Instance MongoDB** : Une base de données locale ou un cluster MongoDB Atlas.

### Installation des dépendances

L'installation complète du projet nécessite les bibliothèques suivantes pour couvrir le frontend, le backend, l'ETL et les tests :

```bash
# Installation des dépendances de production
npm install next react react-dom lucide-react clsx tailwind-merge mongodb sqlite3 drizzle-orm dotenv

# Installation des dépendances de développement
npm install -D typescript @types/node @types/react @types/react-dom postcss autoprefixer tailwindcss drizzle-kit ts-node vitest @testing-library/react @testing-library/jest-dom jsdom
```
...
Créez un fichier `.env` à la racine et renseignez vos accès MongoDB :
```env
...
Générez la table SQLite locale via l'outil Drizzle :
```bash
npx drizzle-kit push:sqlite
```

4. **Exécution du pipeline ETL** :
Ce script collecte 300 produits, les enrichit dans MongoDB et peuple la base SQL :
```bash
...
npm run dev
```
![Lancement de l'application](project_running.png)
...
* **Pipeline complet** : Test de bout en bout sur un échantillon réduit pour confirmer l'intégrité du flux (du format brut MongoDB au format final SQL).
